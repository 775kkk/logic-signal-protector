...
* В шаге 1.6 сервис сам валидирует JWT как resource‑server и принимает запросы напрямую. `api‑gateway` остаётся identity/auth‑сервисом и не проксирует API.
