...
### 1.2 Межсервисная безопасность (проверка jwt token в остальных сервисах)

**Цель:** downstream сервисы должны уметь валидировать jwt token от gateway.
...
3) `api-gateway-service` (идентификация)
   - хранит `external_accounts(providerCode, externalUserId)`
   - выдаёт jwt token с `uid`, `roles`, `perms`
